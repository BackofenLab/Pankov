...
namespace LocARNA {
    ...
    Scoring::Scoring(const Sequence &seqA_,
        ...
        : params(&params_),
          ...
          ext_rna_dataA(
              (ExtRnaData &)rna_dataA_), // TODO: Is this down casting safe?
          ext_rna_dataB(
              (ExtRnaData &)rna_dataB_), // TODO: Is this down casting safe?
          seqA(seqA_),
          seqB(seqB_),
          lambda_(0),
    ...
    void
    Scoring::set_closing_arcs(const Arc &closingA_, const Arc &closingB_) {
        //		std::cout << "set_closing_arcs : " << closingA_.left() <<
        //"," << closingA_.right() << "   " <<closingB_.left() << "," <<
        //closingB_.right() << std::endl;
    ...
    */
    ...
    score_t
    Scoring::arcmatch(const Arc &arcA,
                      const Arc &arcB,
                      bool stacked,
                      bool non_cond) const {
        ...
        if (conditonal_scores) { // Use conditional-prob scores
            //		std::cout << ext_rna_dataA.arc_in_loop_cutoff_prob() <<
            //std::endl;
            //		std::cout << "params->tau_factor: " << params->tau_factor <<
            //std::endl;
            //		assert (sequence_contribution == 0); //TODO: Temporary accept
            //tau to be only zero

            //
            //		double probA = rna_dataA.arc_prob(arcA.left(),
            //arcA.right());
            //		double probB = rna_dataB.arc_prob(arcB.left(),
            //arcB.right());
            ...
            //			std::cout << "sequence_contribution: " <<
            //sequence_contribution << std::endl;
            double cond_zero_penalty = -5; //-9.21; // ~=ln(0.0001)
            double scoreA = cond_zero_penalty;
            double scoreB = cond_zero_penalty;

            if (closingA.left() == 0 &&
                closingA.right() == seqA.length() + 1) { // TODO: And or OR?
                //		 if (non_cond){
                double probExtA =
                    ext_rna_dataA.arc_external_prob(arcA.left(), arcA.right());
                scoreA = (probExtA == 0) ? cond_zero_penalty : log(probExtA);

            } else {
                double joint_probA =
                    ext_rna_dataA.arc_in_loop_prob(arcA.left(), arcA.right(),
                                                   closingA.left(),
                                                   closingA.right());
                double prob_closingA =
                    rna_dataA.arc_prob(closingA.left(), closingA.right());

                //			 std::cout << "=======A " <<  log
                //(joint_probA/prob_closingA);
                if (prob_closingA != 0 && joint_probA != 0)
                    scoreA = log(joint_probA / prob_closingA);
            }

            if (closingB.left() == 0 &&
                closingB.right() == seqB.length() + 1) { // TODO: And or OR?
                //		 if (non_cond){
                double probExtB =
                    ext_rna_dataB.arc_external_prob(arcB.left(), arcB.right());
                scoreB = (probExtB == 0) ? cond_zero_penalty : log(probExtB);

            } else {
                double joint_probB =
                    ext_rna_dataB.arc_in_loop_prob(arcB.left(), arcB.right(),
                                                   closingB.left(),
                                                   closingB.right());
                double prob_closingB =
                    rna_dataB.arc_prob(closingB.left(), closingB.right());
                //			 std::cout << "=======B " <<  log
                //(joint_probB/prob_closingB) << std::endl;
                if (prob_closingB != 0 && joint_probB != 0)
                    scoreB = log(joint_probB / prob_closingB);
            }

            //		 std::cout << "ret_score: " << ret_score << " Score: " <<
            //params->struct_weight * (10.0+ret_score) << std::endl;
            //		 if (non_cond){

            if ((closingA.left() == 0 &&
                 (closingA.right() == seqA.length() + 1)) ||
                (closingB.left() == 0 &&
                 closingB.right() == seqB.length() + 1)) {
                return (score_t)(params->struct_weight *
                                 (3 + scoreA + scoreB)) +
    ...
    score_t
    Scoring::arcDel_conditional(const Arc &arcX,
                                const Sequence &seqX,
                                const RnaData &rna_dataX,
                                const ExtRnaData &ext_rna_dataX,
                                const Arc &closingX,
                                double p0) const {
        ...
        if (closingX.left() == 0 &&
            closingX.right() == seqX.length() + 1) { // TODO: And or OR?
            double probExtX =
                ext_rna_dataX.arc_external_prob(arcX.left(), arcX.right());
            scoreX = (probExtX == 0) ? cond_zero_penalty : log(probExtX);
        } else {
            double joint_probX =
                ext_rna_dataX.arc_in_loop_prob(arcX.left(), arcX.right(),
                                               closingX.left(),
                                               closingX.right());
            double prob_closingX =
                rna_dataX.arc_prob(closingX.left(), closingX.right());
    ...
    Scoring::arcDel(const Arc &arcX,
        ...
        if (!params->mea_scoring) {
            if (conditonal_scores) { // Use conditional-prob scores
                ...
                if (isA)
                    ret_cond =
                        arcDel_conditional(arcX, seqA, rna_dataA, ext_rna_dataA,
                                           closingA, params->exp_probA) +
                        loop_indel_score(gapX(arcX.left(), isA) +
                                         gapX(arcX.right(), isA));
                else
                    ret_cond =
                        arcDel_conditional(arcX, seqB, rna_dataB, ext_rna_dataB,
                                           closingB, params->exp_probB) +
                        loop_indel_score(gapX(arcX.left(), isA) +
                                         gapX(arcX.right(), isA));
                //		std::cout << "   ret_cond:" << ret_cond <<
                //std::endl;
                return ret_cond;
            } else {
                return
                    ...
                    loop_indel_score(
                        gapX(arcX.left(), isA) +
                        gapX(arcX.right(), isA)) + // score of aligining
                                                   // base-pair ends wo gap,
                                                   // such that it is multiply
                                                   // by gamma_loop ratio
                    (stacked
                         ? (isA ? stack_weightsA[arcX.idx()]
