...
namespace LocARNA {
    ...
    AlignerNN::AlignerNN(const AlignerNN &a)
        : params(new AlignerNParams(*a.params)),
          ...
          traceback_closing_arcA(
              a.traceback_closing_arcA), // TODO: What to set as index?
          traceback_closing_arcB(a.traceback_closing_arcB) {}

    AlignerNN::AlignerNN(const AlignerParams &ap_)
        : params(new AlignerNParams(dynamic_cast<const AlignerNParams &>(
              ap_))), // TODO: Check if needed to switch to AlignerNNParams?
          ...
          def_scoring_view(this),
          mod_scoring_view(this),
          traceback_closing_arcA(0,
                                 0,
                                 params->seqA_
                                     ->length()), // TODO: What to set as index?
        ...
        IAmat.resize(mapper_arcsA.get_max_info_vec_size() + 1,
                     bpsB.num_bps() + 1); //+1 for empty arcs
        IBmat.resize(bpsA.num_bps() + 1, mapper_arcsB.get_max_info_vec_size() +
                         1); //+1 for empty arcs

        IADmat.resize(bpsA.num_bps() + 1,
                      bpsB.num_bps() + 1); //+1 for empty arcs
        IADmat.fill(infty_score_t::neg_infty);

        IBDmat.resize(bpsA.num_bps() + 1,
                      bpsB.num_bps() + 1); //+1 for empty arcs
        IBDmat.fill(infty_score_t::neg_infty);

        M.resize(mapper_arcsA.get_max_info_vec_size() + 1,
                 mapper_arcsB.get_max_info_vec_size() + 1);
        Emat.resize(mapper_arcsA.get_max_info_vec_size() + 1,
                    mapper_arcsB.get_max_info_vec_size() + 1);
        Fmat.resize(mapper_arcsA.get_max_info_vec_size() + 1,
                    mapper_arcsB.get_max_info_vec_size() + 1);

        gapCostAmat.resize(seqA.length() + 3, seqA.length() + 3);
        gapCostBmat.resize(seqB.length() + 3, seqB.length() + 3);

        trace_debugging_output =
    ...
    void
    AlignerNN::computeGapCosts(bool isA, ScoringView sv) {
    ...
    AlignerNN::getGapCostBetween(pos_type leftSide,
                                 pos_type rightSide,
                                 bool isA)
    // todo: Precompute the matrix?!
    {
        ...
        //	if (leftSide >= rightSide && rightSide == 0) // empty arc for
        //domain ins
    ...
    template <class ScoringView>
    infty_score_t
    AlignerNN::compute_IX(ArcIdx idxX,
                          const Arc &arcY,
                          matidx_t i_index,
                          bool isA,
                          ScoringView sv) {
        ...
        const SparsificationMapper &mapper_arcsX =
            isA ? mapper_arcsA : mapper_arcsB;
        ...
        seq_pos_t i_prev_seq_pos =
            mapper_arcsX.get_pos_in_seq_new(idxX, i_index - 1);
        // TODO: Check border i_index==1,0
        const ArcIdxVec &arcIdxVecX =
            mapper_arcsX.valid_arcs_right_adj(idxX, i_index);
        ...
        for (ArcIdxVec::const_iterator arcIdx = arcIdxVecX.begin();
            ...
            //	    std::cout << ">>> arc_indel_score_extend=IXD(" << arcX <<
            //"," << arcY<< ")"<< IXD(arcX, arcY, isA) << "+" <<
            //sv.scoring()->arcDel(arcX, isA) << "+" << gap_score
            //	    		<< "="<< arc_indel_score_extend << "
            //arc_indel_score_open:" << arc_indel_score_open << std::endl;
        }
    ...
    template <class ScoringView>
    infty_score_t
    AlignerNN::compute_E_entry(seq_pos_t al_seq_pos,
                               matidx_t i_index,
                               matidx_t j_index,
                               seq_pos_t i_seq_pos,
                               seq_pos_t i_prev_seq_pos,
    ...
    template <class ScoringView>
    infty_score_t
    AlignerNN::compute_M_entry(ArcIdx idxA,
                               ArcIdx idxB,
                               seq_pos_t al_seq_pos,
                               seq_pos_t bl_seq_pos,
                               matidx_t i_index,
                               matidx_t j_index,
                               ScoringView sv) {
        ...
        seq_pos_t i_prev_seq_pos =
            mapper_arcsA.get_pos_in_seq_new(idxA, i_index - 1);
        // TODO: Check border i_index==1,0
        seq_pos_t j_prev_seq_pos =
            mapper_arcsB.get_pos_in_seq_new(idxB, j_index - 1);
        // TODO: Check border j_index==1,0
        if (trace_debugging_output) {
            std::cout << "compute_M_entry al_seq_pos: " << al_seq_pos
                      << " bl_seq_pos: " << bl_seq_pos << "i:" << i_seq_pos
                      << " i_index: " << i_index << " i_prev:" << i_prev_seq_pos
                      << " j: " << j_seq_pos << " j_index:" << j_index
                      << " j_prev:" << j_prev_seq_pos << std::endl;
        }
        ...
        // compute_M_entry
        Emat(i_index, j_index) = compute_E_entry(al_seq_pos, i_index, j_index,
                                                 i_seq_pos, i_prev_seq_pos, sv);
        max_score =
            std::max(max_score, (tainted_infty_score_t)Emat(i_index, j_index));

        // base ins
        Fmat(i_index, j_index) = compute_F_entry(bl_seq_pos, i_index, j_index,
                                                 j_seq_pos, j_prev_seq_pos, sv);
        max_score =
            std::max(max_score, (tainted_infty_score_t)Fmat(i_index, j_index));

        // list of valid arcs ending at i/j
        const ArcIdxVec &arcsA =
            mapper_arcsA.valid_arcs_right_adj(idxA, i_index);
        const ArcIdxVec &arcsB =
            mapper_arcsB.valid_arcs_right_adj(idxB, j_index);
        ...
        if (params->multiloop_deletion_ > 0) {
            ...
            for (ArcIdxVec::const_iterator arcAIdx = arcsA.begin();
                ...
                if (params->multiloop_deletion_ <
                    (arcA.right() - arcA.left() + 1)) // Todo: if the adjlist is
                                                      // sorted we can return
                                                      // instead of continue
                    continue;
                matidx_t arcA_left_index_before =
                    mapper_arcsA.first_valid_mat_pos_before(idxA, arcA.left(),
                                                            al_seq_pos);
                seq_pos_t arcA_left_seq_pos_before =
                    mapper_arcsA.get_pos_in_seq_new(idxA,
                                                    arcA_left_index_before);

                opening_cost_A = 0;
                //			if (arcA_left_seq_pos_before < (arcA.left()
                //- 1)) {
                //			// implicit base deletion because of
                //sparsification
                //			opening_cost_A =
                ...
                infty_score_t arc_indel_score_open = sv.D(arcA, empty_arcB) +
                    (infty_score_t)(sv.scoring()->loop_indel_score(
                ...
                tainted_infty_score_t domain_del_score = arc_indel_score_open +
                    opening_cost_A + M(arcA_left_index_before, j_index);
                max_score = std::max(max_score, domain_del_score);
                if (trace_debugging_output) {
                    std::cout
                        << "M: domain del: arcA" << arcA
                        << " D(arcA,empty_arcB)=" << sv.D(arcA, empty_arcB)
                        << " sv.scoring()->arcDel(arcA, true)="
                        << sv.scoring()->arcDel(arcA, true) << "M("
                        << arcA_left_index_before << "," << j_index
            ...
            for (ArcIdxVec::const_iterator arcBIdx = arcsB.begin();
                ...
                if (params->multiloop_deletion_ <
                    (arcB.right() - arcB.left() + 1)) // Todo: if the adjlist is
                                                      // sorted we can return
                                                      // instead of continue
                    continue;

                matidx_t arcB_left_index_before =
                    mapper_arcsB.first_valid_mat_pos_before(idxB, arcB.left(),
                                                            bl_seq_pos);
                seq_pos_t arcB_left_seq_pos_before =
                    mapper_arcsB.get_pos_in_seq_new(idxB,
                                                    arcB_left_index_before);
                opening_cost_B = 0;
                //			if (arcB_left_seq_pos_before < (arcB.left()
                //- 1)) {
                //			// implicit base deletion because of
                //sparsification
                //			opening_cost_B =
                ...
                tainted_infty_score_t domain_ins_score = arc_indel_score_open +
                    opening_cost_B + M(i_index, arcB_left_index_before);
                max_score = std::max(max_score, domain_ins_score);
                if (trace_debugging_output) {
                    std::cout
                        << "M: domain ins: arcB" << arcB
                        << " D(arcA,empty_arcB)=" << sv.D(empty_arcA, arcB)
                        << " sv.scoring()->arcDel(arcB, true)="
                        << sv.scoring()->arcDel(arcB, false) << "M(" << i_index
                        << "," << arcB_left_index_before
                ...
                if (arc_match_score > max_score) {
                    ...
                    if (trace_debugging_output) {
                        std::cout << "compute_M_entry arcs " << arcA << " , "
                                  << arcB
                                  << "arc match score: " << arc_match_score
                                  << std::endl;
                    }
                }
            }
    ...
    AlignerNN::init_M_E_F(ArcIdx idxA, ArcIdx idxB, ScoringView sv) {
        ...
        for (matidx_t i_index = 1;
             i_index < mapper_arcsA.number_of_valid_mat_pos(idxA); i_index++) {
            seq_pos_t i_seq_pos =
                mapper_arcsA.get_pos_in_seq_new(idxA, i_index);
        ...
        for (matidx_t j_index = 1;
             j_index < mapper_arcsB.number_of_valid_mat_pos(idxB); j_index++) {
            seq_pos_t j_seq_pos =
                mapper_arcsB.get_pos_in_seq_new(idxB, j_index);
            if (!indel_score.is_neg_infty()) { // checked for optimization
    ...
    void
    AlignerNN::fill_IB_entries(Arc arcA, ArcIdx idxB, pos_type max_br) {
        if (trace_debugging_output)
            std::cout << "fill_IB_entries: "
                      << "arcA=" << arcA << ", idxB=" << idxB
                      << "max_br=" << max_br << std::endl;
        ...
        for (pos_type j_index = 1; j_index < max_right_index;
             j_index++) { // limit entries due to trace control

            IBmat(arcA.idx(), j_index) =
                compute_IX(idxB, arcA, j_index, false, def_scoring_view);
            // std::cout << "IBmat( << " << arcA.idx() << "," <<  j_index <<
            // ")=" << IBmat(arcA.idx(), j_index) << std::endl;
        ...
        //		std::cout << "fill_IB_entries arcA: " << arcA << " bl: "<<
        //bl <<  " IBmat: " << std::endl << IBmat << std::endl;
    }

    // compute/align matrix M
    void
    AlignerNN::fill_M_entries(const Arc &arcA, const Arc &arcB) {
        assert(params->track_closing_bp_); // Track the exact closing right ends
                                           // of al and bl
        ...
        if (trace_debugging_output) {
            std::cout << "fill_M_entries: arcs: " << arcA << "  " << arcB
                      << std::endl;
        }
        // initialize M
        ...
        // std::cout << "max_ij_index set to " << max_i_index << " " <<
        // max_j_index << std::endl;
        for (matidx_t i_index = 1; i_index < max_i_index; i_index++) {
            /*
            //tomark: constraints
            ...
            */
            for (matidx_t j_index = 1; j_index < max_j_index; j_index++) {
                // E and F matrix entries will be computed by compute_M_entry
                M(i_index, j_index) =
                    compute_M_entry(idxA, idxB, arcA.left(), arcB.left(),
                                    i_index, j_index, def_scoring_view);
    ...
    AlignerNN::fill_D_entry(const Arc &arcA, const Arc &arcB) {
        ...
        if (trace_debugging_output) {
            std::cout << "compute_D_entry(arcA:" << arcA << " arcB:" << arcB
                      << std::endl;
        }
        UnmodifiedScoringViewNN sv = def_scoring_view;

        matidx_t ar_prev_mat_idx_pos =
            mapper_arcsA.number_of_valid_mat_pos(idxA) -
            1; // TODO: VERY IMPORTNANT: -1 or not?

        matidx_t br_prev_mat_idx_pos =
            mapper_arcsB.number_of_valid_mat_pos(idxB) - 1;

        seq_pos_t ar_prev_seq_pos =
            mapper_arcsA.get_pos_in_seq_new(idxA, ar_prev_mat_idx_pos);
        infty_score_t jumpGapCostA =
            getGapCostBetween(ar_prev_seq_pos, ar_seq_pos, true);
        seq_pos_t br_prev_seq_pos =
            mapper_arcsB.get_pos_in_seq_new(idxB, br_prev_mat_idx_pos);
        ...
        assert(IADmat(idxA, idxB) == infty_score_t::neg_infty ||
               IADmat(idxA, idxB) == ia);
        assert(IBDmat(idxA, idxB) == infty_score_t::neg_infty ||
               IBDmat(idxA, idxB) == ib);
        IADmat(idxA, idxB) = ia; // TODO: avoid recomputation
        IBDmat(idxA, idxB) = ib; // TODO: avoid recomputation
        if (trace_debugging_output) {
            std::cout << "Set IAD(" << arcA.idx() << "," << arcB.idx()
                      << ") =" << ia << std::endl;
            std::cout << "Set IBD(" << arcA.idx() << "," << arcB.idx()
                      << ") =" << ib << std::endl;
            std::cout << "m=" << m << " ia=" << ia
                      << " ib=" << IBmat(arcA.idx(), br_prev_mat_idx_pos) << "+"
                      << jumpGapCostB << "=" << ib << std::endl;
        }
        ...
        if (trace_debugging_output)
            std::cout << "DD[" << arcA << "," << arcB << "]:" << D(arcA, arcB)
                      << std::endl;
    }

    // Fill entries of domain insertion deletion i.e. IA with empty B
    // sub-sequence and the opposite
    void
    AlignerNN::compute_IAB_entries_domain(seq_pos_t start,
                                          seq_pos_t end,
                                          bool isA) {
        const SparsificationMapper &mapper_arcsX =
            (isA) ? mapper_arcsA : mapper_arcsB;
        ...
        for (pos_type xl = end + 1; xl > start;) {
            xl--;
            if (trace_debugging_output)
                std::cout << "align_D xl: " << xl << std::endl;

            const BasePairs::LeftAdjList &adjlX = bspX.left_adjlist(xl);
            if (adjlX.empty()) {
                if (trace_debugging_output)
                    std::cout << "empty left_adjlist(xl=)" << xl << std::endl;
                continue;
            }
            Arc empty_arcY = BasePairs__Arc(bpsY.num_bps(), 0, 0);
            for (BasePairs::LeftAdjList::const_iterator arcX = adjlX.begin();
                 arcX != adjlX.end(); ++arcX) {
                if (params->multiloop_deletion_ <
                    (arcX->right() - arcX->left() + 1)) // Todo: if the adjlist
                                                        // is sorted we can
                                                        // return instead of
                                                        // continue
                    continue;
                if (trace_debugging_output) {
                    std::cout << "align_D domain insertion isA=" << isA
                              << "  arcX():" << *arcX << std::endl;
                    std::cout << "fill_IA_entries: " << xl << ","
                              << arcX->right() << std::endl;
                ...
                matidx_t xr_prev_mat_idx_pos =
                    mapper_arcsX.number_of_valid_mat_pos(arcX->idx()) -
                    1; // TODO: VERY IMPORTNANT: -1 or not?
                seq_pos_t xr_prev_seq_pos =
                    mapper_arcsX.get_pos_in_seq_new(arcX->idx(),
                                                    xr_prev_mat_idx_pos);
                infty_score_t jumpGapCostX =
                    (infty_score_t)scoring->loop_indel_score(
                        getGapCostBetween(xr_prev_seq_pos, arcX->right(), isA)
                            .finite_value());
                if (isA) {
                    infty_score_t ix =
                        IAmat(xr_prev_mat_idx_pos, empty_arcY.idx()) +
                        jumpGapCostX;
                    IADmat(arcX->idx(), empty_arcY.idx()) = ix;
                    D(*arcX, empty_arcY) = ix;
                } else {
                    infty_score_t ix =
                        IBmat(empty_arcY.idx(), xr_prev_mat_idx_pos) +
                        jumpGapCostX;
    ...
    AlignerNN::align_D() {
        ...
        for (pos_type al = r.endA() + 1; al > r.startA();) {
            ...
            //	pos_type max_bl =
            //std::min(r.endB(),params->trace_controller.max_col(al)); //tomark:
            //trace_controller
            //	pos_type min_bl =
            ...
            for (pos_type bl = max_bl + 1; bl > min_bl;) {
                ...
                // pos_type max_ar=adjlA.begin()->right();	//tracecontroller
                // not considered
                ...
                for (BasePairs::LeftAdjList::const_iterator arcA =
                    ...
                    for (BasePairs::LeftAdjList::const_iterator arcB =
                        ...
                        if (trace_debugging_output) {
                            std::cout << "align_D arcA:" << *arcA
                                      << ", arcB:" << *arcB << std::endl;
                            std::cout << "fill_M_entries: " << al << ","
                                      << arcA->right() << "  " << bl << ","
                                      << arcB->right() << std::endl;
                            std::cout << "                 arcA:" << *arcA
                                      << "  arcB" << *arcB << std::endl;
                        ...
                        fill_M_entries(*arcA, *arcB); // fill_M_entries(al,
                                                      // arcA->right(), bl,
                                                      // arcB->right());
                    ...
                    // TODO: Make the IA and IB calculations with exact right
                    // side(?)
                    // from aligner.cc: find maximum arc ends
                    /*			pos_type max_ar = al;
                                            pos_type max_br = bl;

                                            // get the maximal right ends of any
                       arc match with left ends (al,bl)
                                            // in noLP mode, we don't consider
                       cases without immediately enclosing arc match
                                            arc_matches.get_max_right_ends(al,
                       bl, &max_ar, &max_br,
                       ...
                       (BasePairs::LeftAdjList::const_iterator arcB =
                                                            adjlB.begin(); arcB
                       != adjlB.end(); ++arcB) {
                                                    scoring->set_closing_arcs(BasePairs__Arc(0,
                       al, max_ar) , *arcB); //TODO: Verfiy this, specially the
                       constructor and idx
                                                    std:cout <<
                       "fill_IA_entries: " << al << "," << max_ar << "  " <<
                       arcB->left() << "," << arcB->right();

                                                    fill_IA_entries(al, *arcB,
                       max_ar);
                                            }
                       ...
                       (BasePairs::LeftAdjList::const_iterator arcA =
                                                            adjlA.begin(); arcA
                       != adjlA.end(); ++arcA) {
                                                    scoring->set_closing_arcs(*arcA,
                       BasePairs__Arc(0, bl, max_br)); //TODO: Verfiy this,
                       specially the constructor and idx
                                                    fill_IB_entries(*arcA, bl,
                       max_br);
                                            }
    ...
    AlignerNN::align() {
        ...
        if (params->sequ_local_) {
            ...
            matidx_t last_index_A =
                mapper_arcsA.number_of_valid_mat_pos(bpsA.num_bps()) - 1;
            seq_pos_t last_valid_seq_pos_A =
                mapper_arcsA.get_pos_in_seq_new(bpsA.num_bps(), last_index_A);
            matidx_t last_index_B =
                mapper_arcsB.number_of_valid_mat_pos(bpsB.num_bps()) - 1;
            seq_pos_t last_valid_seq_pos_B =
                mapper_arcsB.get_pos_in_seq_new(bpsB.num_bps(), last_index_B);
            if (trace_debugging_output) {
            ...
            scoring->set_closing_arcs(BasePairs__Arc(bpsA.num_bps(), 0,
                                                     seqA.length() + 1),
                                      BasePairs__Arc(bpsB.num_bps(), 0,
                                                     seqB.length() +
                                                         1)); // TODO: check it
            std::cout << "align top level" << std::endl;

            fill_M_entries(BasePairs__Arc(bpsA.num_bps(), 0, seqA.length() + 1),
                           BasePairs__Arc(bpsB.num_bps(), 0,
                                          seqB.length() + 1));
            ...
            if (trace_debugging_output)
                std::cout << "M matrix: " << bpsA.num_bps() << " ,"
                          << last_index_A << "  , " << bpsB.num_bps() << ","
                          << last_index_B << std::endl
                          << M << std::endl;
            if (trace_debugging_output) {
    ...
    template <class ScoringView>
    void
    AlignerNN::trace_IX(ArcIdx idxX,
                        matidx_t i_index,
                        const Arc &arcY,
                        bool isA,
                        ScoringView sv) {
        ...
        for (ArcIdxVec::const_iterator arcIdx = arcIdxVecX.begin();
            ...
            if (gap_score.is_finite()) { // convert the base gap score to the
                ...
                gap_score = (infty_score_t)(
                    sv.scoring()->loop_indel_score(gap_score.finite_value()));
                //			 std::cout << "IBDmat(arcY.idx(),arcX.idx())
                //before:" << IBDmat(arcY.idx(),arcX.idx()) <<std::endl;

                //			std::cout << "*** sv.D(" << arcX << "," <<
                //arcY << "," <<  isA << ")=" << sv.D(arcX, arcY, isA) <<
                //std::endl;
                //			assert (! (IXD(arcX, arcY, isA) ==
                ...
                //			std::cout << "arc_indel_score_open:" <<
                //arc_indel_score_open << "=" << sv.D(arcX, arcY, isA)  << "+"
                //<<  sv.scoring()->arcDel(arcX, isA) << "+" <<  gap_score <<
                //"+" << sv.scoring()->indel_opening_loop() << std::endl;
                //			std::cout << "arc_indel_score_extend: " <<
                //arc_indel_score_extend << "=" << IXD(arcX, arcY, isA) << "+"
                //<<  sv.scoring()->arcDel(arcX, isA) << "+" <<  gap_score <<
                //std::endl;
                if (IX(i_index, arcY, isA) == arc_indel_score_extend) {
                    ...
                    if (isA) {
                        ...
                        //				    IADmat(arcX.idx(),arcY.idx())
                        //= sv.D(arcX, arcY); //tocheck: donna know why :-D
                        ...
                        //				    IBDmat(arcY.idx(),arcX.idx())
                        //= sv.D(arcY, arcX); //tocheck: donna know why :-D
    ...
    template <class ScoringView>
    void
    AlignerNN::trace_IXD(const Arc &arcA,
                         const Arc &arcB,
                         bool isA,
                         ScoringView sv) {
        if (trace_debugging_output)
        ...
        {
            matidx_t ar_prev_mat_idx_pos =
                mapper_arcsA.first_valid_mat_pos_before(idxA, ar_seq_pos,
                                                        arcA.left());
            seq_pos_t ar_prev_seq_pos =
                mapper_arcsA.get_pos_in_seq_new(idxA, ar_prev_mat_idx_pos);
            infty_score_t jumpGapCostA =
            ...
            sv.scoring()->set_closing_arcs(traceback_closing_arcA,
                                           traceback_closing_arcB);
            fill_IA_entries(idxA, arcB, ar_seq_pos);
            //		std::cout << "    IAD:" << IADmat(arcA.idx(), arcB.idx()) <<
        ...
        {
            matidx_t br_prev_mat_idx_pos =
                mapper_arcsB.first_valid_mat_pos_before(
                    idxB, br_seq_pos, arcB.left()); // tocheck: ar or ar-1?
            seq_pos_t br_prev_seq_pos =
                mapper_arcsB.get_pos_in_seq_new(idxB, br_prev_mat_idx_pos);
            infty_score_t jumpGapCostB =
                getGapCostBetween(br_prev_seq_pos, br_seq_pos, false);
            ...
            if (trace_debugging_output)
                std::cout << "IXD(" << arcA.idx() << "," << arcB.idx()
                          ...
                          << " ?== " << IB(arcA, br_prev_mat_idx_pos) << "+"
                          << jumpGapCostB << std::endl;
    ...
    template <class ScoringView>
    void
    AlignerNN::trace_D(const Arc &arcA, const Arc &arcB, ScoringView sv) {
        assert(!params->no_lonely_pairs_); // take special care of noLP in this
                                           // method

        if (trace_debugging_output)
            std::cout << "****trace_D****" << arcA << " " << arcB
                      << "sv.D(arcA, arcB)=" << sv.D(arcA, arcB) << std::endl;
        assert(!params->struct_local_);
        ...
        if (params->multiloop_deletion_ > 0) {
            ...
            if (arcB.idx() == bpsB.num_bps()) {
                ...
                if (sv.D(arcA, arcB) == ia) {
                    if (trace_debugging_output)
                        std::cout << "     trace_D domain deletion"
                                  << std::endl;

                    trace_IX(idxA, ar_prev_mat_idx_pos, arcB, true, sv);
                    for (size_type k = ar_prev_seq_pos + 1; k < ar_seq_pos;
                         k++) {
                        alignment.append(k, -1);
                    }
                    return;
                ...
                //				D(*arcA, empty_arcB) =
                //IAmat(arcA->right()-1, empty_arcB.idx());
            ...
            if (arcA.idx() == bpsA.num_bps()) {
                fill_IB_entries(arcA, idxB, br_seq_pos);
                matidx_t br_prev_mat_idx_pos =
                    mapper_arcsB.number_of_valid_mat_pos(idxB) -
                    1; // TODO: VERY IMPORTNANT: -1 or not?
                seq_pos_t br_prev_seq_pos =
                    mapper_arcsB.get_pos_in_seq_new(idxB, br_prev_mat_idx_pos);
                infty_score_t jumpGapCostB =
                ...
                if (sv.D(arcA, arcB) == ib) {
                    if (trace_debugging_output)
                        std::cout << "     trace_D domain insertion"
                                  << std::endl;

                    trace_IX(idxB, br_prev_mat_idx_pos, arcA, false, sv);
                    for (size_type k = br_prev_seq_pos + 1; k < br_seq_pos;
                         k++) {
                        alignment.append(-1, k);
                    }
                    return;
        ...
        matidx_t br_prev_mat_idx_pos = mapper_arcsB.first_valid_mat_pos_before(
            idxB, br_seq_pos, arcB.left()); // tocheck: ar or ar-1?
        seq_pos_t br_prev_seq_pos =
            mapper_arcsB.get_pos_in_seq_new(idxB, br_prev_mat_idx_pos);
        infty_score_t jumpGapCostB =
        ...
        if (sv.D(arcA, arcB) ==
            (infty_score_t)(gap_score + opening_cost_B + inner_score +
                            Emat(ar_prev_mat_idx_pos, br_prev_mat_idx_pos))) {
            trace_E(idxA, ar_prev_mat_idx_pos, idxB, br_prev_mat_idx_pos, false,
                    def_scoring_view);
        } else if (sv.D(arcA, arcB) ==
                   (infty_score_t)(
                       gap_score + opening_cost_A + inner_score +
                       Fmat(ar_prev_mat_idx_pos, br_prev_mat_idx_pos))) {
            trace_F(idxA, ar_prev_mat_idx_pos, idxB, br_prev_mat_idx_pos, false,
                    def_scoring_view);
        } else if (sv.D(arcA, arcB) ==
                   (infty_score_t)(gap_score + opening_cost_A + inner_score +
                                   opening_cost_B + M(ar_prev_mat_idx_pos,
                                                      br_prev_mat_idx_pos))) {
            //		std::cout << "traceD-M " << arcA << "  " << arcB <<
            //std::endl;
            trace_M(idxA, ar_prev_mat_idx_pos, idxB, br_prev_mat_idx_pos, false,
                    def_scoring_view);
    ...
    AlignerNN::trace_E(ArcIdx idxA,
        ...
        if (trace_debugging_output)
            std::cout << "******trace_E***** "
                      << " idxA:" << idxA << " idxB:" << idxB
                      << " i:" << i_seq_pos << " :: " << Emat(i_index, j_index)
                      << std::endl;

        seq_pos_t i_prev_seq_pos = mapper_arcsA.get_pos_in_seq_new(
            idxA, i_index - 1); // TODO: Check border i_index==1,0
    ...
    AlignerNN::trace_F(ArcIdx idxA,
        ...
        if (trace_debugging_output) {
            std::cout << "******trace_F***** "
                      << " idxA:" << idxA << " idxB:" << idxB
                      << " j:" << j_seq_pos << " :: " << Fmat(i_index, j_index)
                      << std::endl;
        }

        seq_pos_t j_prev_seq_pos = mapper_arcsB.get_pos_in_seq_new(
            idxB, j_index - 1); // TODO: Check border j_index==1,0
    ...
    pos_type
    AlignerNN::get_arc_leftend(ArcIdx &idxX, bool isA) {
        if (isA) {
    ...
    template <class ScoringView>
    void
    AlignerNN::trace_M_noex(ArcIdx idxA,
                            matidx_t i_index,
                            ArcIdx idxB,
                            matidx_t j_index,
                            bool top_level,
        ...
        if (i_seq_pos > al)
            i_prev_seq_pos = mapper_arcsA.get_pos_in_seq_new(
                idxA, i_index - 1); // TODO: Check border i_index==1,0
        seq_pos_t j_prev_seq_pos = bl;
        if (j_seq_pos > bl)
            j_prev_seq_pos = mapper_arcsB.get_pos_in_seq_new(
                idxB, j_index - 1); // TODO: Check border j_index==1,0
        ...
        //	 std::cout << i_index << "-" << j_index << " " << i_seq_pos <<
        //"-" << j_seq_pos << "  " << al << "-" << bl <<std::endl;
        ...
        if (params->multiloop_deletion_ > 0) {
            ...
            for (ArcIdxVec::const_iterator arcAIdx = arcsA.begin();
                ...
                if (M(i_index, j_index) == domain_del_score) {
                    if (trace_debugging_output)
                        std::cout << "domain del M" << arcA << std::endl;

                    trace_M(idxA, arcA_left_index_before, idxB, j_index,
                            top_level, sv);
            ...
            for (ArcIdxVec::const_iterator arcBIdx = arcsB.begin();
                ...
                if (M(i_index, j_index) == domain_ins_score) {
                    if (trace_debugging_output)
                        std::cout << "domain ins M" << arcB << std::endl;

                    trace_M(idxA, i_index, idxB, arcB_left_index_before,
                            top_level, sv);
        ...
        for (ArcIdxVec::const_iterator arcAIdx = arcsA.begin();
            ...
            for (ArcIdxVec::const_iterator arcBIdx = arcsB.begin();
                 arcBIdx != arcsB.end(); ++arcBIdx) {
                const Arc &arcB = bpsB.arc(*arcBIdx);
                // std::cout << "trace_M_noex: arcA=" << arcA << " arcB=" <<
                // arcB << std::endl;
                matidx_t arcB_left_index_before =
                    mapper_arcsB.first_valid_mat_pos_before(idxB, arcB.left(),
                                                            bl);
                ...
                if (M(i_index, j_index) ==
                    ...
                    trace_E(idxA, arcA_left_index_before, idxB,
                ...
                // arc match, then continue with insertion case
                    ...
                    trace_F(idxA, arcA_left_index_before, idxB,
                            arcB_left_index_before, top_level, sv);
                ...
                // arc match, then continue with general M case
                    ...
                    trace_M(idxA, arcA_left_index_before, idxB,
                            arcB_left_index_before, top_level, sv);
    ...
    AlignerNN::trace_M(ArcIdx idxA,
        ...
        if (trace_debugging_output)
            std::cout << "******trace_M***** "
                      << " idxA:" << idxA << " i:" << i_seq_pos
                      << " idxB:" << idxB << " j:" << j_seq_pos
    ...
    AlignerNN::trace(ScoringView sv) {
        ...
        matidx_t last_mat_idx_pos_A =
            mapper_arcsA.number_of_valid_mat_pos(bpsA.num_bps()) -
            1; // tocheck: check the correctness
        // seq_pos_t last_seq_pos_A = mapperA.get_pos_in_seq_new(ps_al,
        // last_mat_idx_pos_A);

        //	seq_pos_t ps_bl = r.startB() - 1;
        matidx_t last_mat_idx_pos_B =
            mapper_arcsB.number_of_valid_mat_pos(bpsB.num_bps()) -
            1; // tocheck: check the correctness
        // seq_pos_t last_seq_pos_B = mapperB.get_pos_in_seq_new(ps_bl,
        // last_mat_idx_pos_B);

        traceback_closing_arcA = Arc(bpsA.num_bps(), 0, seqA.length() +
                                         1); // TODO: What to set as index?
        traceback_closing_arcB = Arc(bpsB.num_bps(), 0, seqB.length() +
                                         1); // TODO: What to set as index?

        trace_M(bpsA.num_bps(), last_mat_idx_pos_A, bpsB.num_bps(),
                last_mat_idx_pos_B, true,
