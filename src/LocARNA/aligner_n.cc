...
namespace LocARNA {
    ...
    AlignerN::fill_IA_entries(pos_type al, Arc arcB, pos_type max_ar) {
        ...
        if (params->track_closing_bp_) { // Track the exact closing right ends
                                         // of al and bl
            max_right_index =
                mapperA.first_valid_mat_pos_before(al, max_ar) + 1;
        } else {
        ...
        for (matidx_t i_index = 1; i_index < max_right_index; i_index++) {
            ...
            seq_pos_t i_prev_seq_pos =
                mapperA.get_pos_in_seq_new(al, i_index - 1);
            if (!params->track_closing_bp_ &&
                bpsA.exists_arc(al, i_seq_pos)) // TODO: not sure if yet needed
                                                // in non track_bp either
            {
                const Arc &arcA = bpsA.arc(al, i_seq_pos);
                IADmat(arcA.idx(), arcB.idx()) =
                    ((IAmat(i_index - 1, arcB.idx()))) +
                    getGapCostBetween(i_prev_seq_pos, i_seq_pos, true);
                //		    std::cout << "      IADmat(" << arcA.idx() <<
                //"," << arcB.idx() << ")=" <<  IADmat(arcA.idx(), arcB.idx())
                //<< std::endl;
            }
        }
    ...
    AlignerN::fill_IB_entries(Arc arcA, pos_type bl, pos_type max_br) {
        ...
        if (params->track_closing_bp_) { // Track the exact closing right ends
                                         // of al and bl
            max_right_index =
                mapperB.first_valid_mat_pos_before(bl, max_br) + 1;
        } else {
        ...
        for (pos_type j_index = 1; j_index < max_right_index;
            ...
            if (!params->track_closing_bp_ &&
                bpsB.exists_arc(bl, j_seq_pos)) // TODO: not sure if yet needed
                                                // in non track_bp either
            {
    ...
    AlignerN::fill_M_entries(pos_type al,
        ...
        if (trace_debugging_output) {
            std::cout << "fill_M_entries: " << al << "," << ar << "  " << bl
                      << "," << br << std::endl;
        }
        // initialize M
        ...
        if (params->track_closing_bp_) { // Track the exact closing right ends
            ...
            matidx_t max_i_index = (al == 0)
                ? mapperA.number_of_valid_mat_pos(al)
                : mapperA.first_valid_mat_pos_before_eq(al, ar - 1) + 1;
            matidx_t max_j_index = (bl == 0)
                ? mapperB.number_of_valid_mat_pos(bl)
                : mapperB.first_valid_mat_pos_before_eq(bl, br - 1) + 1;
            for (matidx_t i_index = 1; i_index < max_i_index; i_index++) {
                ...
                */
                for (matidx_t j_index = 1; j_index < max_j_index; j_index++) {
                    // E and F matrix entries will be computed by
                    // compute_M_entry
                    M(i_index, j_index) =
                        compute_M_entry(al, bl, i_index, j_index,
                                        def_scoring_view);
                    // toask: where should we care about non_default scoring
                    // views
    ...
    AlignerN::align_D() {
        ...
        for (pos_type al = r.endA() + 1; al > r.startA();) {
            ...
            for (pos_type bl = max_bl + 1; bl > min_bl;) {
                ...
                if (!params->track_closing_bp_) { // Original SPARSE recursion
                                                  // for max right ends
                    // ------------------------------------------------------------
                    ...
                    // get the maximal right ends of any arc match with left
                    // ends (al,bl)
                ...
                } else { // Track the exact closing right ends of al and bl
                    ...
                    for (BasePairs::LeftAdjList::const_iterator arcA =
                        ...
                        for (BasePairs::LeftAdjList::const_iterator arcB =
                            ...
                            // compute matrix M
                            //	    stopwatch.start("compM");
                            fill_M_entries(al, arcA->right(), bl,
                                           arcB->right());
    ...
    AlignerN::align() {
        ...
        if (params->sequ_local_) {
            ...
            scoring->set_closing_arcs(BasePairs__Arc(0, 0, seqA.length() + 1),
                                      BasePairs__Arc(0, 0, seqB.length() +
                                                         1)); // TODO: check it
            ...
            if (trace_debugging_output)
                std::cout << "M matrix: " << ps_al << "," << last_index_A
    ...
    AlignerN::trace(ScoringView sv) {
        ...
        traceback_closing_arcA =
            Arc(0, 0, seqA.length() + 1); // TODO: What to set as index?
        traceback_closing_arcB =
            Arc(0, 0, seqB.length() + 1); // TODO: What to set as index?
