...
namespace LocARNA {
    ...
    private:
        ...
        //! computes the datastructures for sparsification mapping based on
        //! indexing the arcs including pseudoarc of external case
        void
    ...
    public:
        ...
        SparsificationMapper(const BasePairs &bps_,
            ...
            if (index_left_ends) {
                ...
                //			compute_mapping_idx_arcs();
                std::cout << "Caution compute_mapping_idx_arcs_external is "
                             "called, Exparna-p compatibility needs to be "
                             "verified"
                          << std::endl;
                compute_mapping_idx_arcs_external();
            }
    ...
    private:
        ...
        is_valid_arc(const Arc &inner_arc,
            ...
            if (conditional) {
                ...
                double joint_prob =
                    rnadata.arc_in_loop_prob(inner_arc.left(),
                                             inner_arc.right(), arc.left(),
                                             arc.right());
                if (outer_prob == 0)
                    return false;
                return (joint_prob / outer_prob >= 0.001);
            } else
                return rnadata.arc_in_loop_prob(inner_arc.left(),
                                                inner_arc.right(), arc.left(),
                                                arc.right()) >=
    ...
    public:
        ...
        is_valid_pos(const Arc &arc,
            ...
            if (conditional) {
                double outer_prob = rnadata.arc_prob(arc.left(), arc.right());
                double joint_prob =
                    rnadata.unpaired_in_loop_prob(pos, arc.left(), arc.right());
                if (outer_prob == 0)
                    return false;
                ...
                return rnadata.unpaired_in_loop_prob(pos, arc.left(),
                                                     arc.right()) >=
                    prob_unpaired_in_loop_threshold; // todo: additional
