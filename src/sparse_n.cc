...
struct command_line_parameters {
    //! only pairs with a probability of at least min_prob are taken into
    //! account
    double min_prob;
    ...
    bool opt_galaxy_xml;   //!< whether to print a galaxy xml wrapper for the
                           //!parameters
    ...
    bool opt_special_gap_symbols; //!< whether to use special gap symbols in the
                                  //!alignment result
    ...
    bool opt_track_closing_bp; //!< whether to track right end of a closing
                               //!basepair
    bool opt_use_conditional_scoring; //!< whether to use the conditional
                                      //!probability scoring
    int opt_multiloop_deletion; //!< whether to allow aligning an entire branch
                                //!of a multiloop to gap
    ...
    double min_am_prob; //!< only matched arc-pair with a probability of at
                        //!least min_am_prob are taken into account
    double min_bm_prob; //!< only matched base-pair with a probability of at
                        //!least min_bm_prob are taken into account

    bool opt_subopt; //!< find suboptimal solution (either k-best or all
                     //!solutions better than a threshold)
...
option_def my_options[] = {
    ...
    {"galaxy-xml", 0, &clp.opt_galaxy_xml, O_NO_ARG, 0, O_NODEFAULT, "",
     "Galaxy xml wrapper"},
    {"version", 'V', &clp.opt_version, O_NO_ARG, 0, O_NODEFAULT, "",
     "Version info"},
    {"verbose", 'v', &clp.opt_verbose, O_NO_ARG, 0, O_NODEFAULT, "", "Verbose"},

    {"", 0, 0, O_SECTION, 0, O_NODEFAULT, "", "Scoring_parameters"},

    {"match", 'm', 0, O_ARG_INT, &clp.match_score, "50", "score",
     "Match score"},
    {"mismatch", 'M', 0, O_ARG_INT, &clp.mismatch_score, "0", "score",
     "Mismatch score"},
    {"ribosum-file", 0, 0, O_ARG_STRING, &clp.ribosum_file, "RIBOSUM85_60", "f",
     "Ribosum file"},
    {"use-ribosum", 0, 0, O_ARG_BOOL, &clp.use_ribosum, "true", "bool",
     "Use ribosum scores"},
    {"indel", 'i', 0, O_ARG_INT, &clp.indel_score, "-350", "score",
     "Indel score"},
    {"indel-loop", 'i', 0, O_ARG_INT, &clp.indel_score_loop, "-350", "score",
     "Indel score for loops"},
    {"indel-opening", 0, 0, O_ARG_INT, &clp.indel_opening_score, "-600",
     "score", "Indel opening score"},
    {"indel-opening-loop", 0, 0, O_ARG_INT, &clp.indel_opening_loop_score,
     "-900", "score", "Indel opening score for loops"},
    {"struct-weight", 's', 0, O_ARG_INT, &clp.struct_weight, "200", "score",
     "Maximal weight of 1/2 arc match"},
    {"exp-prob", 'e', &clp.opt_exp_prob, O_ARG_DOUBLE, &clp.exp_prob,
     O_NODEFAULT, "prob", "Expected probability"},
    {"tau", 't', 0, O_ARG_INT, &clp.tau_factor, "100", "factor",
     "Tau factor in percent"},

    {"track-closing-bp", 0, &clp.opt_track_closing_bp, O_NO_ARG, 0, O_NODEFAULT,
     "", "Track right end of a closing basepair "},
    {"use-conditional-scoring", 0, &clp.opt_use_conditional_scoring, O_NO_ARG,
     0, O_NODEFAULT, "", "Use conditional probability scoring "},
    {"multiloop-deletion", 0, 0, O_ARG_INT, &clp.opt_multiloop_deletion, "0",
     "diff",
     "Maximum allowed length of a  multiloop branch to be aligned gap, "
     "value 0 disables computation "},

    //    {"exclusion",'E',0,O_ARG_INT,&clp.exclusion_score,"0","score","Exclusion
    //    weight"},
    //    {"stacking",0,&clp.opt_stacking,O_NO_ARG,0,O_NODEFAULT,"","Use
    //    stacking terms (needs stack-probs by RNAfold -p2)"},
    //    {"new-stacking",0,&clp.opt_newstacking,O_NO_ARG,0,O_NODEFAULT,"","Use
    //    new stacking terms (needs stack-probs by RNAfold -p2)"},

    /*    {"",0,0,O_SECTION,0,O_NODEFAULT,"","Type of locality"},

          {"struct-local",0,0,O_ARG_BOOL,&clp.struct_local,"false","bool","Structure
       local"},
          {"sequ-local",0,0,O_ARG_BOOL,&clp.sequ_local,"false","bool","Sequence
       local"},
          {"free-endgaps",0,0,O_ARG_STRING,&clp.free_endgaps,"----","spec","Whether
       and which end gaps are free. order: L1,R1,L2,R2"},
          {"normalized",0,&clp.opt_normalized,O_ARG_INT,&clp.normalized_L,"0","L","Normalized
       local alignment with parameter L"},
    */

    {"", 0, 0, O_SECTION, 0, O_NODEFAULT, "", "Controlling_output"},

    {"width", 'w', 0, O_ARG_INT, &clp.output_width, "120", "columns",
     "Output width"},
    {"clustal", 0, &clp.opt_clustal_out, O_ARG_STRING, &clp.clustal_out,
     O_NODEFAULT, "file", "Clustal output"},
    {"pp", 0, &clp.opt_pp_out, O_ARG_STRING, &clp.pp_out, O_NODEFAULT, "file",
     "PP output"},

    //    {"local-output",'L',&clp.opt_local_output,O_NO_ARG,0,O_NODEFAULT,"","Output
    //    only local sub-alignment"},
    //    {"pos-output",'P',&clp.opt_pos_output,O_NO_ARG,0,O_NODEFAULT,"","Output
    //    only local sub-alignment positions"},
    {"write-structure", 0, &clp.opt_write_structure, O_NO_ARG, 0, O_NODEFAULT,
     "", "Write guidance structure in output"},
    {"special-gap-symbols", 0, &clp.opt_special_gap_symbols, O_NO_ARG, 0,
     O_NODEFAULT, "",
     "Special distinct gap symbols for loop gaps or gaps caused by "
     "sparsification"},

    {"stopwatch", 0, &clp.opt_stopwatch, O_NO_ARG, 0, O_NODEFAULT, "",
    ...
    //    {"kbest",0,&clp.opt_subopt,O_ARG_INT,&clp.kbest_k,"-1","k","Enumerate
    //    k-best alignments"},
    //    {"better",0,&clp.opt_subopt,O_ARG_INT,&clp.subopt_threshold,"-1000000","t","Enumerate
    //    alignments better threshold t"},

    {"", 0, 0, O_SECTION, 0, O_NODEFAULT, "", "MEA_score controlling options"},

    {"mea-alignment", 0, &clp.opt_mea_alignment, O_NO_ARG, 0, O_NODEFAULT, "",
     "Do MEA alignment"},
    {"probcons-file", 0, &clp.opt_probcons_file, O_ARG_STRING,
     &clp.probcons_file, O_NODEFAULT, "file", "Probcons parameter file"},

    {"match-prob-method", 0, 0, O_ARG_INT, &clp.match_prob_method, "0", "int",
     "Method for computation of match probs"},
    {"temperature", 0, 0, O_ARG_INT, &clp.temperature, "150", "int",
     "Temperature for PF-computation"},
    {"pf-struct-weight", 0, 0, O_ARG_INT, &clp.pf_struct_weight, "200",
     "weight", "Structure weight in PF-computation"},
    ...
    {"probability-scale", 0, 0, O_ARG_INT, &clp.probability_scale, "10000",
     "scale", "Scale for probabilities/resolution of mea score"},

    {"write-match-probs", 0, &clp.opt_write_matchprobs, O_ARG_STRING,
     &clp.matchprobs_file, O_NODEFAULT, "file",
     "Write match probs to file (don't align!)"},
    {"read-match-probs", 0, &clp.opt_read_matchprobs, O_ARG_STRING,
     &clp.matchprobs_file, O_NODEFAULT, "file",
    ...
    {"noLP", 0, &clp.no_lonely_pairs, O_NO_ARG, 0, O_NODEFAULT, "",
     "No lonely pairs"},
    //    {"ignore-constraints",0,&clp.opt_ignore_constraints,O_NO_ARG,0,O_NODEFAULT,"","Ignore
    //    constraints in pp-file"},
...
main(int argc, char **argv) {
    ...
    if (clp.opt_help) {
        cout << "sparse - a tool for pairwise fast alignment of RNAs" << endl
             << endl;
    ...
    */
    ...
    if (clp.opt_use_conditional_scoring && !clp.opt_track_closing_bp) {
        std::cerr << "Error: conditonal scoring only works if track_closing_bp "
                     "is enabled"
                  << std::endl;
    }
    ...
    PFoldParams pfparams(clp.no_lonely_pairs,
                         clp.opt_stacking || clp.opt_new_stacking, -1,
                         2); // bpspan disabled

    ExtRnaData *rna_dataA = 0;
    try {
        rna_dataA =
            new ExtRnaData(clp.fileA, clp.min_prob,
    ...
    try {
        rna_dataB =
            new ExtRnaData(clp.fileB, clp.min_prob,
                           clp.prob_basepair_in_loop_threshold,
                           clp.prob_unpaired_in_loop_threshold,
                           clp.max_bps_length_ratio, clp.max_uil_length_ratio,
                           clp.max_bpil_length_ratio, pfparams);
    ...
    if (clp.max_diff_pw_alignment != "" && clp.max_diff_alignment_file != "") {
        std::cerr << "Cannot simultaneously use both options "
                     "--max-diff-pw-alignemnt and --max-diff-alignment-file."
                  << std::endl;
        return -1;
    }
    ...
    //
    // (needed for mea alignment with probabilistic consistency transformation
    // of arc match scores)
    //
    if (clp.opt_read_arcmatch_scores || clp.opt_read_arcmatch_probs) {
        if (clp.opt_verbose) {
        ...
        arc_matches =
            new ArcMatches(seqA, seqB, clp.arcmatch_scores_file,
    ...
    //    SparsificationMapper mapperB(bpsB, *rna_dataB,
    //    clp.prob_unpaired_in_loop_threshold,
    //    clp.prob_basepair_in_loop_threshold, true);

    // TODO: It is inefficient to create mapper_arcsX, if track closing pair is
    // not enabled
    // construct mappers where right_sdj list is indexed by arcIndex
    SparsificationMapper mapper_arcsA(bpsA, *rna_dataA,
                                      clp.prob_unpaired_in_loop_threshold,
                                      clp.prob_basepair_in_loop_threshold,
                                      false);
    SparsificationMapper mapper_arcsB(bpsB, *rna_dataB,
                                      clp.prob_unpaired_in_loop_threshold,
                                      clp.prob_basepair_in_loop_threshold,
                                      false);
    ...
    if (clp.opt_read_matchprobs && !clp.opt_mea_alignment) {
        std::cerr << "Warning: clp.opt_read_matchprobs ignored for non-mea "
                     "alignment.\n";
    }
    ...
    if (clp.opt_write_matchprobs || clp.opt_mea_alignment) {
        ...
        if (!clp.use_ribosum) {
            std::cerr << "WARNING: Ribosum scoring used for mea_alignment and "
                         "computing matchprobs."
                      << std::endl;
        }

        if (clp.opt_read_matchprobs) {
            match_probs->read_sparse(clp.matchprobs_file, seqA.length(),
                                     seqB.length());
        } else {
            if (clp.match_prob_method == 1) {
                ...
                if (clp.opt_verbose) {
                    std::cout << "Compute match probabilities using pairHMM."
                              << std::endl;
                }
    ...
    ScoringParams scoring_params(
        clp.match_score, clp.mismatch_score,
        ...
        (clp.opt_mea_alignment && !clp.opt_mea_gapcost) ? 0 : clp.indel_score *
                (clp.opt_mea_gapcost ? clp.probability_scale / 100 : 1),
        (clp.opt_mea_alignment && !clp.opt_mea_gapcost) ? 0
        ...
        ribosum, ribofit,
        0, // unpaired_weight
        clp.struct_weight, clp.tau_factor, clp.exclusion_score, my_exp_probA,
        my_exp_probB, clp.temperature, clp.opt_stacking, clp.opt_new_stacking,
        clp.opt_mea_alignment, clp.mea_alpha, clp.mea_beta, clp.mea_gamma,
    ...
    if (clp.opt_normalized) {
        std::cerr << "ERROR: Normalized alignment not supported." << std::endl;
        return -1;

        // 	// do some option consistency checks and output errors
        // 	if (clp.struct_local) {
        // 	    std::cerr
        // 		<< "ERROR: Normalized structure local alignment not
        // supported."
        // 		<<std::endl
        // 		<< "LocARNA ignores struct_local option."<<std::endl;
        // 	    return -1;
        // 	}
        // 	if (!clp.sequ_local) { // important: in the Aligner class, we
        // rely on this
        // 	    std::cerr
        // 		<< "ERROR: Normalized alignment requires option
        // --sequ_local."<<std::endl;
        // 	    return -1;
        // 	}
        // //	score =
        // aligner.normalized_align(clp.normalized_L,clp.opt_verbose);
    ...
    if (clp.opt_normalized || DO_TRACE) { // if we did a trace (one way or
        ...
        if (clp.opt_clustal_out) {
            ...
            if (out.good()) {
                ...
                // for legacy, clustal files of pairwise alignments contain the
                // score
                if (seqA.num_of_rows() == 1 && seqB.num_of_rows() == 1)
                    out << " --- Score: " << score;
                ...
                if (clp.opt_write_structure) {
                    // annotate multiple alignment with structures
                    ma.prepend(MultipleAlignment::SeqEntry(
                        "", alignment.dot_bracket_structureA(false)));
                    ma.append(MultipleAlignment::SeqEntry(
                        "", alignment.dot_bracket_structureB(false)));
                }

                ma.write(out, clp.output_width);

            } else {
                cerr << "Cannot write to " << clp.clustal_out << endl
                     << "! Exit.";
                return_code = -1;
            }
        }
        if (clp.opt_pp_out) {
            ...
            if (out.good()) {
                RnaData consensus(*rna_dataA, *rna_dataB,
                                  aligner.get_alignment(), my_exp_probA,
                                  my_exp_probB);
